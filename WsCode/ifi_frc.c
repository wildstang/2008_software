...
void Interrupt_Handler_Low()
{
  ...
  else if (INTCON3bits.INT2IF && INTCON3bits.INT2IE) // external interrupt 1?
  {
    ...
    Int_1_ISR(); // call the interrupt 1 handler (in interrupts.c)
  }
  #endif
  #ifdef ENABLE_INT_2
  else if (INTCON3bits.INT3IF && INTCON3bits.INT3IE) // external interrupt 2?
  {
    ...
    Int_2_ISR(); // call the interrupt 2 handler (in interrupts.c)
  }
  #endif
  ...
  else if (INTCONbits.RBIF && INTCONbits.RBIE) // external interrupts 3 through 6?
  {
    ...
    {
      Int_3_ISR(Port_B & 0x10 ? 1 : 0); // call the interrupt 3 handler (in interrupts.c)
    }
    if(Port_B_Delta & 0x20) // did external interrupt 4 change state?
    {
      Int_4_ISR(Port_B & 0x20 ? 1 : 0); // call the interrupt 4 handler (in interrupts.c)
    }
    if(Port_B_Delta & 0x40) // did external interrupt 5 change state?
    {
      Int_5_ISR(Port_B & 0x40 ? 1 : 0); // call the interrupt 5 handler (in interrupts.c)
    }
    if(Port_B_Delta & 0x80) // did external interrupt 6 change state?
    {
      Int_6_ISR(Port_B & 0x80 ? 1 : 0); // call the interrupt 6 handler (in interrupts.c)
    }
  }
  #endif
